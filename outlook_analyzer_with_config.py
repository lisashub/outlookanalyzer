...
def main():  
    ...
    # # Try to open config file - if no config file, set default
    # try:
    #     f = open("config.ini")
    ...
    #     supress_prompt = (options["supress_prompt"])
    #     max_email_number_to_extract_input = (options["max_email_number_to_extract_input"])
    #     date_start_input = (options["date_start_input"])
    #     date_end_input = (options["date_end_input"])

    #     f.close()

    # except IOError:
    #     supress_prompt = False
